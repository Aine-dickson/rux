...
description = "The four laws, the element audit, and the decisions behind them, with the tradeoffs each one accepted."
weight = 17
+++

<!-- GENERATED FROM docs/01-rationale.md BY site/sync-docs.sh. DO NOT EDIT HERE. -->


Why Rux is shaped the way it is. Read this before proposing a change to the [spec](/reference/spec/): most additions that feel necessary are already answered by one of the laws below, and the whole design holds together only as long as the laws do.
...
Everything in the [spec](/reference/spec/) is downstream of these four. When in doubt, a proposed feature has to pass all four.
...
A `<button>` can `tap`, `press`, `longpress`, `focus`. A `<view>` can `tap`, `drag`, `swipe`, `scroll`. You don't reach for a new widget to get behavior; you bind a handler to a capability the element already has. See [events](/reference/spec/#events) for the capability tables.
...
**Cost:** we inherit some of CSS's verbosity and quirks, and we must clearly document *which subset* we honor (see [the CSS subset](/reference/spec/#styling)) so authors aren't surprised when an obscure property is ignored.
...
**Cost:** `rhai` is Rust-*like*, not Rust: no borrow checker, dynamic types, interpreter overhead. The escape hatch: promote hot or heavy logic down into the compiled host when it stabilizes. See [the host contract](/reference/spec/#scripting-and-the-host).
...
[Architecture](/contribute/) and the v0.3 plan in
[Roadmap](/roadmap/)). Because the tree is thrown away, every piece of
...
Next: the [formal spec](/reference/spec/).
