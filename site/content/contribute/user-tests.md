...
| Does a departing page animate at all? | desktop, purpose-built probe | **Yes, and this is the part that needed a clean test.** Two pages, one tall and one empty, no overlapping text, no scroller: the departing page's band declines smoothly from 753 to 137 over about a second and then flattens. So the engine does animate the leave, inside a scroller and outside it alike. **What fails is legibility, not the mechanism** |
| Why the router example still reads as "only the incoming moves" | — | **Open, and unexplained.** In `examples/router.rux` specifically, navigating `/crew` to `/about` makes the region holding the crew rows drop in one step and stay flat, which the clean probe does not reproduce. Two candidates not yet separated: the two pages' text overlapping in the same space at similar dimness so a crossfade reads as mush, and the document reflowing instantly when the departing page leaves the flow. **Do not record this as passing until it is understood** |
