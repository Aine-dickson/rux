...
| Why the router example still reads as "only the incoming moves" | desktop, window, measured by colour channel | **A real bug, now measured.** The two pages were temporarily recoloured pure red and pure blue so each could be read off its own channel while they overlap. The departing page fades correctly, 255 down to 98 over ~800ms, and is then **cut to 0 at ~845ms of a 1400ms transition, while still 38% visible**. It does not fade away, it thins slightly and pops. The arrival is also wrong at the other end: nothing for ~300ms, then a jump straight to 90 rather than a smooth start. **Cause not yet identified**, and deliberately not guessed at |
