...
<!-- GENERATED FROM docs/06-roadmap.md BY site/sync-docs.sh. DO NOT EDIT HERE. -->


Where Rux goes next. Last updated 2026-08-11, with the **`rux-rhai` fork design**
under v0.7.

For *what works today*, read [As Built](/reference/). This document is
...
- **v0.4.0**: opens once v0.3 is done; first item is pseudo-classes, unblocked by
  reactivity. See the [v0.4 section](#v0-4-drawn-from-the-known-ceilings) below.
...
between this code and [Architecture](/contribute/).
...
The honored set is listed in [As Built](/reference/). Everything else is
...
rationale](/reference/rationale/#ephemeral-ui-state-automatic-by-default-controllable-by-opt-in).
...
[Architecture](/contribute/) always described, delete the category.
...
[Rationale → Ephemeral UI state](/reference/rationale/#ephemeral-ui-state-automatic-by-default-controllable-by-opt-in).
...
3. **Script documentation.** Rux's script layer needs its own docs, *beyond*
   ...
   *cheaper* than the signal-mutation change, it's a lookup-failure path, not a
   ...
   > literals, is under [v0.7](#v0-7-the-script-tier-and-the-fork-then-animation-then-packaging).
...
5. **Animation**, added 2026-08-11. Rux has no transition of any kind, which is
   ...
   - **`transition` on style changes** first and alone. **Built.** It fires
     ...
     [Transitions](/reference/css/) for the surface, and
