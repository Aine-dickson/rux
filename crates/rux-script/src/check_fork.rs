//! The checker as it was before step 3 of `docs/11-next.md`: on the fork's
//! AST and the annotations kept beside it. Debug builds run it beside the
//! checker on Rux's own AST and require the two to say the same thing, until
//! the port is proved and this file is deleted.
...
use crate::check::{Context, Finding, Guess, Seen, Table, Tpl};
...
/// Debug builds: the checker on Rux's AST says what this one says. Findings
/// are compared as sets, since the order within a line is the order they were
/// found in, and the two walk some constructs in a different order.
pub(crate) fn agree(src: &str, cx: &Context, old: &(Vec<Finding>, Table), new: &(Vec<Finding>, Table)) {
    let key = |f: &Finding| (f.line, f.template, f.is_error, f.message.clone());
    let mut a: Vec<_> = old.0.iter().map(key).collect();
    let mut b: Vec<_> = new.0.iter().map(key).collect();
    a.sort();
    a.dedup();
    b.sort();
    b.dedup();
    let seen = |t: &Table| {
        let mut v: Vec<_> = t
            .seen
            .iter()
            .map(|s| (s.template, s.line, s.kind, s.name.clone(), s.path.clone(), s.ty.clone(), s.nullable))
            .collect();
        v.sort();
        v.dedup();
        v
    ...
    let guesses = |t: &Table| {
        let mut v: Vec<_> = t.guesses.iter().map(|g| (g.line, g.function.clone(), g.param.clone(), g.ty.clone())).collect();
        v.sort();
        v
    };
    let (sa, sb) = (seen(&old.1), seen(&new.1));
    let (ga, gb) = (guesses(&old.1), guesses(&new.1));
    if a == b && sa == sb && ga == gb {
        return;
    }
    let gone: Vec<_> = a.iter().filter(|f| !b.contains(f)).collect();
    let came: Vec<_> = b.iter().filter(|f| !a.contains(f)).collect();
    let seen_gone: Vec<_> = sa.iter().filter(|f| !sb.contains(f)).collect();
    let seen_came: Vec<_> = sb.iter().filter(|f| !sa.contains(f)).collect();
    panic!(
        "the checker on Rux's AST and the one on the fork's disagree; a port bug.\n\
         script:\n{src}\ntemplate pieces: {}\n\
         only the fork's checker found: {gone:#?}\nonly Rux's found: {came:#?}\n\
         recorded only by the fork's: {seen_gone:#?}\nrecorded only by Rux's: {seen_came:#?}\n\
         guesses: {ga:?} against {gb:?}",
        cx.template.len()
    );
}
