...
pub fn pack(
    ...
    std::fs::write(&manifest_xml, android_manifest(manifest, dev))
...
/// The `versionCode` Android and the Play Store order releases by, from the
/// app's `version`: `MAJOR * 1_000_000 + MINOR * 1_000 + PATCH`, at least 1.
///
/// It was always 1, and Play refuses an update whose code does not rise, so
/// every release after an app's first would have been rejected. Derived
/// rather than asked for, so it rises whenever the version does and nobody
/// has two numbers to keep in step. A pre-release (`1.0.0-rc.1`) has its
/// release's code, so only one of the two can be uploaded. Minor and patch
/// numbers above 999 do not fit and are clamped, which keeps the code rising
/// within a major version as long as they stay under that.
pub fn version_code(version: &str) -> u32 {
    let core = version.split(['-', '+']).next().unwrap_or_default();
    let mut parts = core.split('.').map(|p| p.trim().parse::<u32>().unwrap_or(0));
    let (major, minor, patch) = (
        parts.next().unwrap_or(0).min(2_100),
        parts.next().unwrap_or(0).min(999),
        parts.next().unwrap_or(0).min(999),
    );
    (major * 1_000_000 + minor * 1_000 + patch).max(1)
}
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    <uses-sdk android:minSdkVersion="{MIN_API}" android:targetSdkVersion="{TARGET_API}" />{internet}
...
"#,
        ...
        code = version_code(&manifest.version),
...
mod tests {
    ...
    fn an_apostrophe_in_a_name_does_not_break_the_manifest() {
        let xml = android_manifest(&manifest("Bob's Tools"), false);
    ...
    fn an_app_with_an_icon_names_the_resources_that_will_be_generated() {
        ...
        let xml = android_manifest(&with_icon, false);
    ...
    /// Hot reload needs a socket, and a socket needs `INTERNET`. A release
    /// build connects to nothing and must not ask for it.
    #[test]
    fn only_a_dev_build_asks_for_the_network() {
    ...
    /// Each release has a larger `versionCode` than the one before, or Play
    /// refuses it.
    #[test]
    fn the_version_code_rises_with_the_version() {
        assert_eq!(version_code("0.1.0"), 1_000);
        assert_eq!(version_code("1.2.3"), 1_002_003);
        assert!(version_code("1.10.0") > version_code("1.9.99"));
        assert!(version_code("2.0.0") > version_code("1.999.999"));
        assert_eq!(version_code("1.0.0-rc.1"), version_code("1.0.0"));
        assert_eq!(version_code("0.0.0"), 1, "Android wants at least 1");
        let mut m = manifest("Task List");
        m.version = "1.2.3".into();
        assert!(android_manifest(&m, false).contains(r#"android:versionCode="1002003""#));
    }
    ...
    /// manifest over one. The first dev manifest said "`rux run --device`" in
    ...
    /// Every change is claimed, so Android never rebuilds the activity in the
    /// same process, which winit cannot survive.
    #[test]
    fn every_configuration_change_is_claimed() {
        let xml = android_manifest(&manifest("Task List"), false);
