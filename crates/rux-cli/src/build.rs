...
/// Run cargo once, for one Android ABI or for the host.
fn compile(work: &Path, options: &Options, abi: Option<crate::android::Abi>) -> Result<(), String> {
    ...
    if let Some(abi) = abi {
        cargo.args(["--target", abi.rust_target]);
    }
    ...
    if !status.success() {
        ...
        return Err(match abi {
            Some(abi) => format!("the generated crate did not build for {}", abi.name),
