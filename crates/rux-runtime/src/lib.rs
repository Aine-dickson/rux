...
impl Document {
    ...
    fn reconcile(&mut self, changed: &HashSet<String>) {
        // Outermost affected parents only — a nested one is regenerated by its
        // ancestor's splice.
...
mod tests {
    ...
    /// full rebuild. A checkbox's model drives its synthetic `checked` class (a
    /// re-cascade, not yet reconcilable), so a change to it is such a case.
    #[test]
    fn patch_declines_on_non_reconcilable_structural() {
        let mut doc = Document::from_source(
            "<template><screen><input type=\"checkbox\" r-model=\"on\" /></screen></template>
             <script>let on = signal(false);</script>",
        )
        .expect("load");
        let changed = doc.engine_mut().run_handler_tracked("on = true");
        assert!(!doc.patch(&changed), "a toggle checked-class change still needs a rebuild");
    }
