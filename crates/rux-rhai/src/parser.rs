...
impl Expr {
    /// RUX DIVERGENCE: every expression can be a condition, so this no longer
    /// rejects anything. See crates/rux-rhai/DIVERGENCE.md.
    ///
    /// Upstream this refused a condition whose type was knowable at parse time
    /// and was not `bool`, so `if 0 { … }` was "Expecting a boolean expression,
    /// not a number". Under JavaScript truthiness that is not a mistake, it is
    /// the language, and the check contradicts what evaluation now does.
    ///
    /// Kept as a function that always succeeds rather than deleted, so its eight
    /// call sites stay exactly as upstream wrote them and the next rebase has
    /// one hunk to reconcile instead of eight.
...
/// RUX DIVERGENCE: how long an arrow function's parameter list is, in tokens,
/// counting the `=>` itself. `None` if this is not an arrow function.
///
/// Pure lookahead: nothing is consumed, so a caller that gets `None` carries on
/// as though this had never been asked. That is the whole point, since the
/// shapes being told apart (`(a, b) => …` against `(a + b)`) are identical until
/// several tokens in.
///
/// Only names are accepted as parameters. JS allows defaults and destructuring
/// in that position; neither exists anywhere else in this language, and
/// accepting them here would be inventing rather than matching.
#[cfg(not(feature = "no_function"))]
fn arrow_params_len(input: &mut TokenStream) -> Option<usize> {
    let arrow_at = |input: &mut TokenStream, n: usize| {
        matches!(input.peek_nth(n), Some((Token::DoubleArrow, ..)))
    };

    match input.peek() {
        // `x => …`, the form nearly every callback is written in.
        Some((Token::Identifier(..), ..)) => return arrow_at(input, 1).then_some(2),
        // `() => …`. The tokenizer reads `()` as one token, not two.
        Some((Token::Unit, ..)) => return arrow_at(input, 1).then_some(2),
        Some((Token::LeftParen, ..)) => (),
        _ => return None,
    }

    // `( a , b ) => …`. Walking the list rather than trusting the shape,
    // because everything here is also the beginning of an ordinary
    ...
    loop {
        ...
        match input.peek_nth(n) {
            ...
            Some((Token::RightParen, ..)) => {
                return arrow_at(input, n + 1).then_some(n + 2);
            }
            _ => return None,
        }
    }
...
impl Engine {
    ...
    fn parse_primary(
        ...
        // RUX DIVERGENCE: settled before the match below, not in a match guard.
        // The guard would need `&mut state.input` while the scrutinee still
        // borrows it, and the answer does not depend on the arm anyway.
        #[cfg(not(feature = "no_function"))]
        let is_arrow_fn =
        ...
        let root_expr = match next_token {
            ...
            // RUX DIVERGENCE: arrow functions.
            // See crates/rux-rhai/DIVERGENCE.md.
            //
            // `x => …`, `() => …`, `(x) => …` and `(a, b) => …`, each meaning
            // exactly what the matching `|…|` form means. The parameter list is
            // read here and handed to `parse_anon_fn`, because deciding whether
            // a `(` opens a parameter list or a parenthesised expression means
            // seeing the `=>` behind it, which is several tokens away.
            //
            // Placed above the ordinary handling of these tokens, since every
            // one of them is matched below and the first matching arm wins. It
            // runs only once the lookahead has confirmed the entire shape, so it
            // never consumes anything it cannot use and `(a + b)` never reaches
            // it.
            #[cfg(not(feature = "no_function"))]
            Token::LeftParen | Token::Unit | Token::Identifier(..) if is_arrow_fn => {
                let params = self.take_arrow_params(state)?;
                self.parse_anon_fn(state, settings, false, Some(params))?
            }
            ...
            Token::Pipe | Token::Or if settings.has_option(LangOptions::ANON_FN) => {
                self.parse_anon_fn(state, settings, false, None)?
            }
            ...
            lhs = match (lhs, tail_token) {
                ...
                (Expr::Variable(x, .., pos), t @ (Token::LeftParen | Token::Unit)) => {
                    ...
                    self.parse_fn_call(
                        ...
                        name,
                        no_args,
                        // RUX DIVERGENCE: every call captures the scope it was
                        // written in, so a function can read and write the state
                        // around it. Upstream this is opt-in per call site as
                        // `f!(…)`; here it is what a call means.
                        // See crates/rux-rhai/DIVERGENCE.md.
                        true,
                        #[cfg(not(feature = "no_module"))]
                        ns,
    ...
    fn make_dot_expr(
        ...
        match (lhs, rhs) {
            ...
            (lhs, Expr::FnCall(mut f, func_pos)) => {
                // RUX DIVERGENCE: a method call does not capture the caller's
                // scope, so the flag is cleared here rather than rejected.
                //
                // Upstream this was an error, because capture was opt-in per
                // call site (`f!(…)`) and asking for it in method position was
                // a mistake worth reporting. Here capture is what a call
                // *means*, so by the time a call is discovered to be in method
                // position the flag says nothing about what the author asked
                // for, and erroring would make `colors.len()` illegal.
                //
                // The limitation itself is upstream's and stands: method-call
                // dispatch passes its receiver by reference, and the scope
                // cannot also be borrowed. In practice nothing is lost, since
                // the receiver is the value being worked on and a method that
                // needed the surrounding state would be written as a plain
                // call, which does capture.
                f.capture_parent_scope = false;
    ...
    fn parse_binary_op(
        ...
        loop {
            ...
            let precedence = match current_op {
                ...
                // RUX DIVERGENCE: `++` and `--` are postfix operators on a
                // statement, not binary operators, so the expression simply ends
                // here and `parse_expr_stmt` picks them up. Without this the
                // binary-operator loop rejects them as unknown operators before
                // the statement parser ever sees them.
                Token::Reserved(c) if &**c == "++" || &**c == "--" => return Ok(root),
        ...
        loop {
            ...
            match required_token.as_str() {
                ...
                CUSTOM_SYNTAX_MARKER_FUNC => {
                    ...
                    inputs.push(self.parse_anon_fn(state, settings, skip, None)?);
    ...
    fn parse_expr_stmt(
        ...
        let (op, pos) = match state.input.peek().unwrap() {
            // RUX DIVERGENCE: `x++` and `x--`.
            // See crates/rux-rhai/DIVERGENCE.md.
            //
            // Desugared to `x += 1` / `x -= 1`, so they inherit lvalue checking,
            // operator overloads and everything else `+=` already does. Nothing
            // new reaches evaluation: this is spelling, not semantics.
            //
            // **Statement position only**, deliberately. In JS `x++` is also an
            // expression that evaluates to the value *before* the increment,
            // which is a genuinely confusing rule and the reason `i = i++` is a
            // classic puzzle. rhai's assignment is a statement rather than an
            // expression, so supporting the expression form would mean inventing
            // the confusing half of JS's behaviour rather than matching
            // something that already exists. `x++` as a statement is the form
            // that appears in handlers, which is what this is for.
            (Token::Reserved(s), ..) if &**s == "++" || &**s == "--" => {
                let increment = &**s == "++";
                let pos = state.input.next().unwrap().1;
                let op = if increment { Token::PlusAssign } else { Token::MinusAssign };
                let one = Expr::IntegerConstant(1, pos);
                return Self::make_assignment_stmt(Some(op), state, expr, one, pos);
            }
            // var = ...
    ...
    #[cfg(not(feature = "no_function"))]
    /// RUX DIVERGENCE: consume an arrow function's parameter list and its `=>`,
    ...
    fn take_arrow_params(
        ...
        match state.input.next().unwrap() {
            // `x => …`
            (Token::Identifier(s), ..) => params.push(self.get_interned_string(*s)),
            ...
            (Token::LeftParen, ..) => loop {
                match state.input.next().unwrap() {
                    (Token::Identifier(s), pos) => {
                        let s = self.get_interned_string(*s);
                        // The one thing the lookahead cannot check, because it
                        // is about the list as a whole rather than its shape.
                    ...
                    (token, pos) => {
                        unreachable!("arrow parameter list already checked, got {token:?} at {pos:?}")
                    }
                }
                match state.input.next().unwrap() {
                    (Token::Comma, ..) => (),
            ...
            (token, pos) => {
                unreachable!("not an arrow function, got {token:?} at {pos:?}")
            }
        }
