...
use std::{
    cell::RefCell,
    char,
    // RUX DIVERGENCE: `VecDeque` backs the multi-token lookahead in
    // `TokenStream`. See its doc comment.
...
/// A stream of tokens.
///
/// RUX DIVERGENCE: this was `Peekable<TokenIterator>`, which gives exactly one
/// token of lookahead. See crates/rux-rhai/DIVERGENCE.md.
///
/// One token is not enough to tell an arrow function's parameter list from a
/// parenthesised expression: `(a, b) => …` and `(a)` begin identically, and by
/// the time the `=>` settles it, the names have been consumed and a `Peekable`
/// cannot put them back.
///
/// So this is a `Peekable` that can look further when asked. `next` and `peek`
/// keep their signatures exactly, which is why the ninety-odd call sites in the
/// parser are untouched by the change; [`peek_nth`](Self::peek_nth) is the only
/// addition, and only the arrow-function lookahead uses it.
///
/// Tokens are pulled **on demand**, never eagerly. That matters because the
/// tokenizer is stateful: `TokenizerControl` is toggled by the parser partway
/// through a script (string interpolation is the main one), so reading further
/// ahead than asked could tokenize under settings that have not been applied
/// yet. Buffering only what is actually asked for keeps that bounded to the
/// lookahead itself, which scans identifiers and commas and nothing that
/// interpolation can reach into.
pub struct TokenStream<'a> {
    inner: TokenIterator<'a>,
    /// Tokens read ahead of the cursor, in stream order.
...
impl<'a> TokenStream<'a> {
    ...
    #[inline]
    #[must_use]
    pub fn new(inner: TokenIterator<'a>) -> Self {
    ...
    /// Look at the token `n` places ahead without taking anything, where `0` is
    /// the token [`peek`](Self::peek) would return.
    ///
    /// Pulls exactly as far as asked and no further.
    pub fn peek_nth(&mut self, n: usize) -> Option<&(Token, Position)> {
        while self.buffer.len() <= n {
            match self.inner.next() {
                Some(token) => self.buffer.push_back(token),
                // The iterator is fused and ends with an `EOF`-ish token rather
                // than running dry mid-script, so this is the genuine end.
                None => return None,
            }
        }
        self.buffer.get(n)
