...
impl Dynamic {
    ...
    /// RUX DIVERGENCE: this value as an index, accepting a whole float.
    /// See crates/rux-rhai/DIVERGENCE.md.
    ///
    /// Every number a Rux document handles is an `f64`, because `signal()`
    /// coerces to one. So `rows[i]` where `i` came from a signal handed the
    /// engine a float where it wanted an integer and failed with "Data type
    /// incorrect: f64 (expecting i64)" — indexing by a computed number, which is
    /// most indexing anyone does.
    ///
    /// A **fractional** index is still an error rather than being truncated.
    /// JavaScript answers `undefined` for `arr[1.5]`, which is the one place its
    /// behaviour is not worth copying: silently reading a different element than
    /// the one asked for is the kind of failure this milestone exists to remove.
    #[inline]
    pub fn as_index(&self) -> Result<crate::INT, &'static str> {
        match self.as_int() {
            Ok(i) => Ok(i),
            Err(typ) => match self.as_float() {
                Ok(f) if f.fract() == 0.0 => Ok(f as crate::INT),
                // Deliberately reports the type rather than the value: a
                // fractional index is a mistake in the expression that produced
                // it, and the type is what the reader has to fix.
                _ => Err(typ),
            },
        }
    }
    /// RUX DIVERGENCE: is this value truthy, by JavaScript's rules?
    /// See crates/rux-rhai/DIVERGENCE.md.
    ///
    /// Falsy: `false`, zero of any numeric type, `NaN`, the empty string, and
    /// `()`. **Everything else is truthy**, including an empty array, an empty
    /// object map, the string `"0"` and the string `"false"`.
    ///
    /// This never fails, which is the point: upstream a condition must already
    /// be a `bool` and anything else is a type error. Rux's users arrive from
    /// JavaScript and write `if user { … }` and `r-if="items.length"`, and in
    /// binding position especially they are not thinking about a scripting
    /// language's type rules.
    ///
    /// JavaScript's rules are adopted whole rather than adjusted. An empty array
    /// being truthy is the part that reads worst in isolation, and it is kept
    /// anyway: a rule that is *almost* JavaScript is worse than either following
    /// it or departing from it visibly, because the exception is only ever
    /// discovered by being bitten by it.
    #[must_use]
    pub fn is_truthy(&self) -> bool {
        match self.0 {
            Union::Bool(b, ..) => b,
            Union::Unit(..) => false,
            Union::Str(ref s, ..) => !s.is_empty(),
            Union::Int(n, ..) => n != 0,
            #[cfg(not(feature = "no_float"))]
            Union::Float(n, ..) => *n != 0.0 && !n.is_nan(),
            #[cfg(feature = "decimal")]
            Union::Decimal(ref n, ..) => !n.is_zero(),
            #[cfg(not(feature = "no_closure"))]
            Union::Shared(ref cell, ..) => crate::func::locked_read(cell)
                .map_or(true, |guard| guard.is_truthy()),
            // Arrays, maps, chars, timestamps, function pointers and any
            // registered custom type. JavaScript treats every object as truthy
            // and a one-character string is not empty, so all of these are true.
            _ => true,
        }
    }
