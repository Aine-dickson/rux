...
mod logic_functions {
    ...
    /// RUX DIVERGENCE: `!` on anything, by JavaScript's truthiness rules.
    /// See crates/rux-rhai/DIVERGENCE.md.
    ///
    /// `!items`, `!name` and `!count` all work, and mean what they mean in JS.
    /// A separate overload rather than a change to the one above, so `!` on a
    /// real `bool` still resolves to the cheaper, more specific function.
    ///
    /// Without this, `!` would be the one place a condition still demanded a
    /// `bool`, which is exactly the sort of leftover exception that makes a
    /// language feel arbitrary.
    #[rhai_fn(name = "!")]
    pub fn not_any(x: Dynamic) -> bool {
        !x.is_truthy()
    }
}
