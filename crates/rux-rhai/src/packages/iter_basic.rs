...
// RUX DIVERGENCE (non-semantic): silence a lint, change no behaviour.
//
// `unpredictable_function_pointer_comparisons` fires on the derived `PartialEq`
// because `add` is a function pointer. Upstream 1.25.1 predates the lint being
// on by default, so it warns on every build of the whole workspace. The lint is
// right in general and irrelevant here: these ranges are compared for equality
// only against ranges built the same way, and Rux does not compare them at all.
//
// Left as an `allow` rather than a rewrite. Rewriting the derive would be a real
// change to upstream code carrying no divergence, which is the kind of thing
// that turns a rebase into a merge.
