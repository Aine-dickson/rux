    ...
    fn get_indexed_mut<'t>(
        ...
        match target {
            ...
            Dynamic(Union::Array(arr, ..)) => {
                // val_array[idx]
                let index = idx
                    // RUX DIVERGENCE: a whole f64 indexes too. See DIVERGENCE.md.
                    .as_index()
            ...
            ChainType::Dotting => {
                ...
                match (rhs, new_val, target.as_ref().is_map()) {
                    ...
                    (Expr::Property(x, pos), None, true) => {
                        ...
                        // RUX DIVERGENCE: `?.` guards a missing *property*, not
                        // only an absent base. See crates/rux-rhai/DIVERGENCE.md.
                        //
                        // Upstream, `?.` short-circuits above only when the base
                        // itself is `()`, so on a map that does exist a missing
                        // key raises exactly as `.` would. That is fine while a
                        // missing key yields `()`, and stops being fine the
                        // moment `fail_on_invalid_map_property` is on, which Rux
                        // turns on for every document: the error then has no
                        // opt-out at all. Here `?.` means "absent is an
                        // acceptable answer", whichever part is absent.
                        //
                        // Only `ErrorPropertyNotFound` is swallowed. A `?.` must
                        // not become a blanket catch that hides a real failure
                        // inside the lookup.
                        let item = match self.get_indexed_mut(
                            global, caches, new_scope, this_ptr, obj, index, *pos, op_pos, false,
                            false,
                        ) {
                            Ok(item) => item,
                            Err(err)
                                if parent.options().intersects(ASTFlags::NEGATED)
                                    && matches!(*err, ERR::ErrorPropertyNotFound(..)) =>
                            {
                                return Ok((Dynamic::UNIT, false));
                            }
                            Err(err) => return Err(err),
