...
| **An app looks right on a desktop and wrong on a phone, because safe areas are opt-in** | `env(safe-area-inset-*)` is a CSS value an author has to ask for, exactly as on the web. Nothing pads itself. So a layout developed in a desktop window, where every inset is legitimately zero, draws its header underneath the status bar the first time it reaches a device, and nothing about that is a change in behaviour: the rules never mentioned the edges. **The scaffold `rux new` writes does not use it either**, so the first Android app a new user builds has this fault built in. Found by running the scaffold on an emulator, next to `examples/safe-area.rux`, which uses the insets and clears the bars correctly on the same screen | the scaffold should pad its own bars, and `/reference/css/` beside `env()` | open |
