# 09. Author notes: what is not a bug, and has to be taught
...
| **A two-word prop is passed with the spelling the component reads** | `:id-of="t.id"` does not reach a component reading `id_of`. Tags are kebab and files are snake, so the reasonable guess is that props are too, and they are not. The error blames the component ("`id_of` is not defined") rather than the caller that misspelled it | `/reference/components/` | **fixed 2026-09-24**: props are declared with `prop`, `:id-of` reaches `prop id_of`, and an attribute the component does not declare is an error at the tag, naming the props it has |
| **`to` is a link, not a prop name** | Passing `:to="…"` to a component makes the component a link; it does not arrive as a prop called `to`. Right, and invisible: the component reading `to` reports it undefined. Pass what the link is built from and let the component build it | `/reference/components/`, beside `to` | **fixed 2026-09-24**: a component tag is not an element, so `to` there is an error unless the component declares `prop to;`, in which case it is an ordinary prop |
| **An app looks right on a desktop and wrong on a phone, because safe areas are opt-in** | `env(safe-area-inset-*)` is a CSS value an author has to ask for, exactly as on the web. Nothing pads itself. So a layout developed in a desktop window, where every inset is legitimately zero, draws its header underneath the status bar the first time it reaches a device, and nothing about that is a change in behaviour: the rules never mentioned the edges. **Fixed in the scaffold 2026-09-20**, which now pads the screen by `env(safe-area-inset-top)` and `env(safe-area-inset-bottom)`, so the first Android app someone builds clears the status bar and the gesture bar. Driven on the emulator before and after. The trap itself stays open, because it is about every app an author writes, not about the scaffold: nothing pads itself, and a layout that looks right in a desktop window can be wrong on a phone | the scaffold now shows the shape; `/reference/css/` beside `env()` | open |
...
### From the first physical phone (2026-09-21)

Both rows below were invisible on a desktop and on an emulator, and appeared
within minutes of a real hand touching a real screen.

| Trap | What actually happens, and why it is right | Destination | Status |
|---|---|---|---|
| **A touch coordinate is fractional, so printing one fills the screen** | A drag on hardware reported `total -70.82003784179688,147.15029907226563`, wrapping onto two lines. Nothing is broken: a finger lands between pixels, the physical position is divided by the scale factor, and the result is an `f64` that stringifies at shortest-roundtrip precision, which is exactly what JavaScript prints for the same number. It is invisible everywhere Rux has been driven until now, because **a mouse reports whole pixels** and every synthetic `adb shell input tap` does too, so the same expression yields a tidy `164` on a desktop and seventeen digits on a phone. It bites any author who puts a distance or a coordinate on screen, which the pointer examples all do. **Whether touch coordinates should be rounded before they reach a handler is an open design call**, not a formatting bug | `/reference/script/` beside the event fields, and a design question for the pointer vocabulary | open |
| **`examples/gestures.rux` cannot show the thing it exists to show** | Its own lead text says the finger count "is always 1 unless the hardware is a real touchscreen", and on a real touchscreen it still reads 1. `fingers` is assigned only in `@press`, and `begin_gesture` runs only under `points.len() == 1`, because a second finger deliberately joins the `touches` list rather than starting a competing press. So the count read by `@press` is 1 by construction and can never be anything else. Reading `event.touches.length` in `@drag` reports 3 for three fingers immediately. The engine is right and the example is wrong, which is the worst combination: the one file written to demonstrate multi-touch is the one that denies it works | the example, and `/reference/script/` beside `touches` | open |

### From the security pass (2026-09-24)

| Trap | What actually happens, and why it is right | Destination | Status |
|---|---|---|---|
| **A route that acts when it is opened can be opened by anyone** | With a `scheme` or `link-hosts` in `rux.toml`, any app on the phone and any web page can open the app on any route with any query: `myapp://delete?id=4` arrives exactly as a tap on an in-app link would. That is what a deep link is for, and it is the same on every platform. So a page must not *do* something on arrival that the person did not ask for: show the item and ask, rather than delete it in `mounted`. Route guards run for a link, which is the place to say no, and `linked` is `true` in a guard's scope when the navigation came from outside the app | `/reference/routing/`, beside `scheme` and `link-hosts` | open (a trap for every app, so it stays listed); **`linked` added 2026-09-24**, since a guard had no way to tell a link from a tap |

### From the first real keyboard (2026-09-21)

Found by a user asking a question rather than hitting a bug: *would the keyboard tell a search input from an email one?*

| Trap | What actually happens, and why it is right | Destination | Status |
|---|---|---|---|
| **`<input type="password">` rendered a working field that showed every character** | `type=` is inspected for exactly four values, `radio`, `checkbox`, `textarea` and `select`, and anything else fell through to the plain text path. Nothing validated it, so a type Rux had never heard of was accepted, rendered and typed into, in silence. **This is not the usual shape in this file**: the engine was not right and the author was not wrong. The author wrote the one attribute that means "hide this" and the engine discarded it, which is the worst possible reading of an unknown attribute in the one case where the consequence is a leaked password. An unrecognised `type=` is now an error naming the five that exist. **Which of `search`, `number`, `tel`, `date`, `password`, `email` should become real, and whether they belong in `type=` at all, is an open design call** | the error message carries it; `/reference/inputs/` when the set is decided | **fixed 2026-09-21**, design call open |
| **`items.for_each(|x| total += x)` added up the indexes** | Found probing closures, 2026-09-24. rhai's own `for_each` binds each item to `this` and passes the *index* as the argument, so `x` is 0, 1, 2 and the sum of `[1, 2, 3]` comes out 3. Rux's `forEach` passes the item, as JavaScript's does. Both names resolve, both run, neither warns, and the one that reads like Rust is the one that is wrong for a Rust reader too, since Rust's `for_each` hands over the item. The engine is right by rhai's documentation and the author is right by every other language | `/reference/script/` beside `forEach` (written); whether `for_each` should warn or be reshaped is open | documented, open |
| **A textarea's Enter did nothing on a phone** | `type="textarea"` is documented as multi-line with Enter inserting a newline, and that was true on a desktop only. Android's `EditorInfo` was a constant declaring every field one line deep, so Gboard showed a Done tick and sent an editor action. The engine was right about what a textarea is and wrong about what it told the platform. Worth generalising: **a claim in the spec is a claim per platform**, and the phone is the one where nobody had checked | `/reference/inputs/`, beside the textarea description | **fixed 2026-09-21** |
