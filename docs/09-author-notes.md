...
| Trap | What actually happens, and why it is right | Destination | Status |
|---|---|---|---|
| A component cannot import a file from a parent directory | `use` resolves **downward only**, relative to the importing file: `use components::header` is `components/header.rux` beside it. There is no `super::` and no `..`, and a segment cannot contain a dot, so `src/components/header.rux` has no way to name `src/app.rux`. `use super::app;` looks for a literal `super/` directory. The dependency usually wants to go the other way (the page imports the component), but shared helpers in a parent are simply unreachable | 03-guide, 07-script | **open** |
...
| **How round a box can get is capped by how big it is, and its text is what makes it big** | Reported 2026-08-26: a button reading "Add" would not round past roughly 2.5rem until its width and height were raised. A radius cannot exceed half the shorter side, which is CSS's rule and the only one that can be drawn, and a hug-sized button is only as big as "Add" plus its padding. So the radius is not being ignored, it is being clamped by a box the author sized with text. Reads as a broken property because nothing says the cap exists. **Needs driving in the window before it is closed**, to confirm the clamp is the whole story and that `border-radius: 50%` resolves against the box rather than silently falling back | `/reference/css/`, beside `border-radius` | open |
